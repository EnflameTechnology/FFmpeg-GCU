...
static void print_create_info(AVCodecContext *avctx, 
    ...
    av_log(avctx, AV_LOG_DEBUG, "\t run_mode(%s)              \t\n",
            create_info->run_mode ?
          "TOPSCODEC_RUN_MODE_SYNC" : "TOPSCODEC_RUN_MODE_ASYNC");
    av_log(avctx, AV_LOG_DEBUG, "\t             }\t\n");
}
...
static av_cold int topscodec_decode_init(AVCodecContext *avctx)
{
 ...
 switch (avctx->codec->id) {
    ...
    ret = ctx->topscodec_lib_ctx->lib_topscodecDecGetCaps(ctx->codec_type,
                                                            ctx->card_id,
                                                            ctx->device_id,
                                                            &ctx->caps);
    ...
    if (ctx->enable_crop){
        ...
        if (ctx->crop.top    < 0 || ctx->crop.bottom < 0  ||
            ctx->crop.left   < 0 || ctx->crop.right  < 0  ||
            ctx->crop.top    > avctx->height              ||
            ctx->crop.left   > avctx->width               ||
            ctx->crop.bottom > avctx->height              ||
    ...
    */
    ...
    if (ctx->enable_crop && ctx->enable_rotation) {
        av_log(avctx, AV_LOG_ERROR, "Set Parameter error, Rotation and Crop "
    ...
    if (ctx->enable_crop && ctx->enable_resize) {
        av_log(avctx, AV_LOG_ERROR, "Set Parameter error, Downscale resize "
    ...
    if (ctx->enable_sfo && (ctx->sfo * ctx->sf_idr != 0)) {
        av_log(avctx, AV_LOG_ERROR, "Set Parameter error, Frame sampling"
        " interval and IDR Frame sampling can not be set at the same time.\n");
        ret = AVERROR(EINVAL);
        goto error;
    }
    ...
    if (ctx->enable_resize) {
        ...
        av_log(avctx, AV_LOG_DEBUG, "Setting resize, %dx%d->%dx%d.\n",
            avctx->width, avctx->height, ctx->resize.width, ctx->resize.height);
    }
    ...
    if (ctx->enable_crop) {
        ...
        av_log(avctx, AV_LOG_DEBUG, "Setting crop,src dim:(%dx%d),crop dim:"
                                    "((left,top)(right,bottom)):"
                                    "((%dx%d),(%dx%d))\n",
                                    avctx->width, avctx->height,
                                    ctx->crop.left, ctx->crop.top,
    ...
    if (ctx->enable_sfo) {
        ...
        av_log(avctx, AV_LOG_DEBUG, "Setting sampling interval value,"
                                "sfo:%d,sf_idr:%d\n", ctx->sfo, FF_IDR_MAGIC);
    }

    /*set codec params*/
    ret = ctx->topscodec_lib_ctx->lib_topscodecDecSetParams(ctx->handle,
                                                            &params);
    if (TOPSCODEC_SUCCESS != ret) {
        av_log(avctx, AV_LOG_ERROR,
...
#define TOPSCODECDEC(NAME, LONGNAME, CODEC, BSF_NAME)                          \
    ...
    const AVCodec ff_ ## NAME ## _topscodec_decoder = {                        \
        ...
        .pix_fmts       = (const enum AVPixelFormat[]){AV_PIX_FMT_EFCCODEC,    \
                                                        AV_PIX_FMT_YUV420P,    \
                                                        AV_PIX_FMT_NV12,       \
                                                        AV_PIX_FMT_NV21,       \
                                                        AV_PIX_FMT_RGB24,      \
